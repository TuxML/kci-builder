...
def main():
    ...
    if "storages" not in workers:
        storages = {}
    else:
        storages = workers["storages"]
    for storage in storages:
        name = storage["name"].lower()
        print("Handle %s\n" % name)
        if not "host" in storage:
            host = "local"
        else:
            host = storage["host"]
        workerdir = "output/%s/%s" % (host,name)
        ...
        dockcomp["services"][name] = {}
        dockcomp["services"][name]["hostname"] = name
        dockcomp["services"][name]["ports"] = ["8080:80"]
        path = os.getcwd()
        parent = os.path.join(path, os.pardir)
        parent = os.path.abspath(parent)
        dockcomp["services"][name]["volumes"] = [parent+"/shared_volume:/usr/local/apache2/htdocs/shared_volume"]
        dockcomp["services"][name]["build"] = {}
        dockcomp["services"][name]["build"]["context"] = name
        shutil.copytree("lava-storage",workerdir)
        with open(dockcomposeymlpath, 'w') as f:
            yaml.dump(dockcomp, f)
